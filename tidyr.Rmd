...
Each variable stored in its own cell:  
...
Variables "amount of small particles" and "amount of large particles" are not in their own column. Therefore, doing math with them would be difficult!  
```{r}
pollution
```
...
```{r}
storms
storms2 <- (storms$pressure / storms$wind)
storms2
```
\  
\  
